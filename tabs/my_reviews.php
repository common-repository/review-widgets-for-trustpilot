...
{
update_option( $trustindex_pm_trustpilot->get_option_name('download-timestamp') , time() + (86400 * 2), false);
}
}
...
$wpdb->query("UPDATE `". $trustindex_pm_trustpilot->get_noreg_tablename() ."` SET highlight = '$highlight' WHERE id = '$id'");
}
exit;
}
$reviews = [];
...
');
$download_timestamp = get_option($trustindex_pm_trustpilot->get_option_name('download-timestamp'), time() - 1);
?>
...
<p><?php  echo TrustindexPlugin::___("New reviews loaded!"); ?></p>
		...
		<?php if(!$trustindex_pm_trustpilot->is_trustindex_connected() && $download_timestamp < time()): ?>

<div class="notice notice-error" style="margin: 0 0 15px 0">
<p>
				<?php echo TrustindexPlugin::___("Don't want to waste your time by updating your reviews every week? <a href='%s' target='_blank'>Create a free Trustindex account! »</a>", [ 'https://www.trustindex.io/ti-redirect.php?a=sys&c=wp-trustpilot-l' ]); ?>
...
<td><div class="ti-review-content"><?php  echo $trustindex_pm_trustpilot->getReviewHtml($review); ?></div></td>
<td>
