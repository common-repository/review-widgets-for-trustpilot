...
{
check_admin_referer( 'connect-reg_'.$trustindex_pm_trustpilot->get_plugin_slug());
...
{
check_admin_referer( 'disconnect-reg_'.$trustindex_pm_trustpilot->get_plugin_slug());
delete_option($trustindex_pm_trustpilot->get_option_name("subscription-id"));
...
<p><strong><?php  echo TrustindexPlugin::___('You can connect your %s with your Trustindex account, and can display your widgets easier.', [ 'Widgets for Trustpilot Reviews' ]); ?></strong></p>
...
<input type="hidden" name="command" value="connect" />
			<?php wp_nonce_field( 'connect-reg_'.$trustindex_pm_trustpilot->get_plugin_slug() ); ?>
...
<a class="btn-text" href="https://www.trustindex.io/ti-redirect.php?a=sys&c=wp-trustpilot-4" target="_blank"><?php  echo TrustindexPlugin::___('Create a new Trustindex account');?></a>
</p>
</form>
	<?php endif; ?>
...
<code class="code-ti-w-<?php  echo $wc_i; ?>-<?php  echo $wi_num; ?>">[<?php  echo $trustindex_pm_trustpilot->get_shortcode_name(); ?> data-widget-id="<?php  echo $w['id']; ?>"]</code>
